...
export const UltimateSpeech = async (results: Results) => {
  ...
  const str = `
        ...
        The voting period started on ${new Date(results.startTime).toLocaleDateString()} and concluded on ${new Date(results.endTime).toLocaleDateString()}. During this time, we witnessed robust participation across our states, reflecting the democratic spirit of our people.
